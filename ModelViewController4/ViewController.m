...
- (void)viewDidLoad {
    ...
    timer = [NSTimer scheduledTimerWithTimeInterval:0.1
                                           ...
                                           userInfo:nil
                                            repeats:YES];
    /*
     timer1 = [NSTimer scheduledTimerWithTimeInterval:4
                                              target:self
                                            selector:@selector(runSpinAnimationWithDuration)
                                            userInfo:nil
                                            repeats:YES];
    */
    NSURL *fileURL = [[NSURL alloc] initFileURLWithPath: [[NSBundle mainBundle] pathForResource:self.song.mp3_name ofType:@"mp3"]];
    ...
    backButton.multipleTouchEnabled = YES;
    
    UITapGestureRecognizer*back = [[UITapGestureRecognizer alloc] initWithTarget:self
...
{
     [self runSpinAnimationWithDuration:10*60];
}
-(void) addPicture
{
    ...
    imageView.image = self.song.photo;
...
-(void) addPlayButton
{
    ...
    playButton.center = CGPointMake(self.view.bounds.size.width/2, self.view.bounds.size.height*4.6/5-40);
    [self.view addSubview:playButton];
}
-(void) addBackButton
{
    backButton = [[UIImageView alloc] initWithFrame:CGRectMake(0, 0, 64, 64)];
    backButton.image = [UIImage imageNamed:@"back"];
    backButton.center= CGPointMake(20, 40);
...
- (void) runSpinAnimationWithDuration:(CGFloat) duration;
{
    ...
    rotationAnimation.toValue = [NSNumber numberWithFloat: M_PI * 150  /* full rotation*/  ];
...
-(void) addBackground
{
    ...
    background.frame = CGRectMake(0, 62, self.view.bounds.size.width,self.view.bounds.size.height-62-50);
...
-(void) designSlider
{
    ...
    mySlider.center = CGPointMake(self.view.bounds.size.width/2, self.view.bounds.size.height*4/5-40);
    // set thumbSlider
...
-(void) backToMenu: (UITapGestureRecognizer*) back
{
     [audioPlayer stop];
